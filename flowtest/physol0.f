...
*>***************BOUNDARY SETTINGS*************************************
      ...
      ALLOCATE(P(1:M+2,1:N+2),QST((M-1)*(N-1),(M-1)*(N-1)))
*>PROJSOLA CONTROL PARAMETERS****************************************
...
*>NW POINT
     ...
     1                     (-DT/RE*(1/DY/DY))
      ...
      ENDDO
      WRITE(*,*)'*********FIRST DGESV**********'
      CALL DGESV((M-1)*N,1,UMAT,(M-1)*N,IPIV,
     1                     BU,(M-1)*N,INFO)
c      CALL PLU(UMAT,BU,(M-1)*N)
      WRITE(*,*)'*********FIRST DGESV COMPLETE**'
*>TRANSFORM BU TO U
      DO J=2,N+1
      DO I=2,M
*>NODE1:(I,J)
...
*>NW POINT
     ...
     1                     (-DT/RE*(1/DX/DX-MODEKSI/RAD/2/DX))
      ...
      ENDDO
      ENDDO
      CALL DGESV(M*(N-1),1,VMAT,M*(N-1),IPIV,BV,M*(N-1),INFO)
c      CALL PLU(VMAT,BV,M*(N-1))
*>TRANSFORM BV TO V 
      DO J=2,N
      DO I=2,M+1
      NODE(1)=(I-1)+M*(J-2)
...
*>NW POINT
     ...
     1                   (V(I,J)-V(I,J-1))/DY)
      ...
      ENDDO
      ENDDO
c      CALL PLU(PMAT,BP,(M-1)*(N-1))
      CALL DGESV(M*N,1,PMAT,M*N,IPIV,BP,M*N,INFO)
      DO J=2,N+1
      DO I=2,M+1
      NODE(1)=(I-1)+M*(J-2)
      P(I,J)=BP(NODE(1))
...
*>*************PROJSOLA METHOD*****************************************
      ...
      CALL FIRSTSOLVEUV
      WRITE(*,*)'******FIRSTSOVLEUV COMPLETE*******'
      CALL SECONDSOLVEUV
      WRITE(*,*)'******SECONDSOVLEUV COMPLETE*******'
      CALL SOLVEPRESSURE
      NCOUNT=0
...
*>NW POINT
     ...
     1                     (V(I+1,J)-V(I,J))/DX)
      ...
      ENDDO
      ENDDO
      CALL DGESV((M-1)*(N-1),1,QSTMAT,(M-1)*(N-1),IPIV,
     1                         BQST,(M-1)*(N-1),INFO)
      RETURN
...
101   FORMAT(1X,2F10.5,3ES20.5)
      ...
      DX=X(3,2)-X(2,2);DY=X(2,3)-X(2,2)
      UMAX=MAXVAL(U,0,M+2,1,N+2)
      VMAX=MAXVAL(V,0,M+2,1,N+2)
