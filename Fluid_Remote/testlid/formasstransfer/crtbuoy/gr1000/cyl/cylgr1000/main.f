...
*>B:COMPUTE THE INTERMIDDLE VELOCITY USING IMPLICIT VISCOSITY TERM(FORCE)
      INTEGER(KIND=4)::I,J
      REAL(KIND=8)::RAD
      REAL(KIND=8)::FUX,FUY,FUC,FVX,FVY,FVC,BUOY,VISX,VISY
...
*>U INTERIOR POINTS WITHOUT BOUNDARY POINTS
     ...
     1           MODEKSI*U(I,J)/RAD/RAD)
      UT(I,J)=(VISX-FUX-FUY-FUC)*DT+U(I,J)
      ENDDO
      ENDDO
...
*c      BUOY=0
     ...
     1           MODEKSI/DX/RAD/2.0*(V(I+1,J)-V(I-1,J)))
      VT(I,J)=(VISY+BUOY-FVX-FVY-FVC)*DT+V(I,J)
      ENDDO
      ENDDO
...
*>VELOCITY IN COUPLE WITH CONDENSATION
      ...
      ENDDO
      ENDDO
*>in cylindrical coordinates,using hstcyl
...
*>     1            C,D,N,NBDCND,BDC,BDD,
*>     1            ELMBDA,F,IDIMF,PERTRB,IERROR,W) 
      CALL HSTCYL(A,B,M,MBDCND,BDA,BDB,
     1            C,D,N,NBDCND,BDC,BDD,
     1            ELMBDA,F,IDIMF,PERTRB,IERROR,W) 
...
*>USING HWSCRT SUBROUTINE
      ...
      DO I=2,M
      RAD=XC(I,J)
      F(I,J)=((U(I,J+1)-U(I,J))/DY-(V(I+1,J)-V(I,J))/DX)*RAD-
     1       2.0*0.5*(V(I+1,J)+V(I,J))
      ENDDO
      ENDDO
*>      DO J=2,N
*>      DO I=2,M
...
*>     1            C,D,N,NBDCND,BDC,BDD,
*>     1            ELMBDA,F,IDIMF,PERTRB,IERROR,W) 
      CALL HWSCYL(A,B,M,MBDCND,BDA,BDB,
     1            C,D,N,NBDCND,BDC,BDD,
     1            ELMBDA,F,IDIMF,PERTRB,IERROR,W) 
...
50    FORMAT(1X,A15,I6,1X,A5,1X,F8.4)
      ENDIF
      ENDDO
60    FORMAT(1X,A10,F10.5,1X,A10,F10.8)
