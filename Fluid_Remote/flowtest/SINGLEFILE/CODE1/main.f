...
*>SOUTH WALL
      ...
      UMAT(NODE(1),NODE(5))=1
      BU(NODE(1))=0
*>NORTH WALL
      ELSEIF(J.EQ.N+1)THEN
      NODE(1)=I+1+(M+3)*J
      NODE(4)=I+1+(M+3)*(J-1)
      UMAT(NODE(1),NODE(4))=1
      BU(NODE(1))=2.0*1.0
      ENDIF
*>WEST WALL AND EAST WALL SATISFY THE BOUNDARY REALATION
      ENDDO
      ENDDO
      WRITE(*,*)'BEGIN CALL PLU TO UMAT'
      CALL PLU(UMAT,BU,(M+3)*(N+2))
c      CALL DGESV((M+3)*(N+2),1,UMAT,(M+3)*(N+2),IPIV,BU,
c     1         (M+3)*(N+2),INFO)
c      WRITE(*,*)'INFO=',INFO
*>TRANSFORM BU TO U
      DO J=2,N+1
...
c      CALL DGESV((M+2)*(N+3),1,VMAT,(M+2)*(N+3),IPIV,BV,
c     1         (M+2)*(N+3),INFO)
*>TRANSFORM BV TO V 
      DO J=2,N
      DO I=2,M+1
...
*>*************SOLVE THE NEXT INTERVAL PRESSURE************************
      ...
      REAL(KIND=8)::RAD
      REAL(KIND=8)::PMAT((M+2)*(N+2),(M+2)*(N+2))
      REAL(KIND=8)::BP(1:(M+2)*(N+2))
      INTEGER(KIND=4)::IPIV((M+2)*(N+2)),INFO
      PMAT=0;BP=0
      DO I=1,(M+2)*(N+2)
      PMAT(I,I)=1
      ENDDO
...
*>EAST WALL
      ...
      ENDDO
      ENDDO
      CALL PLU(PMAT,BP,(M+2)*(N+2))
c      CALL DGESV((M+2)*(N+2),1,PMAT,(M+2)*(N+2),IPIV,BP,
c     1         (M+2)*(N+2),INFO)
...
c      CALL DGESV((M+1)*(N+1),1,QSTMAT,(M+1)*(N+1),IPIV,BQST,
c     1         (M+1)*(N+1),INFO)
