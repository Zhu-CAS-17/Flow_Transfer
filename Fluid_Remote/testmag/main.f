...
*>MAGNETIC FIELD:MAGNETIC VECTOR POTENTIAL
      REAL(KIND=8),ALLOCATABLE::FAR(:,:),FAI(:,:)
      REAL(KIND=8),ALLOCATABLE::CURR(:,:)
...
*>MAGNETIC PROPERTIES
      REAL(KIND=8)::OMIGA
      REAL(KIND=8)::ELECOND(:,:),DIELECOEF(:,:),MAGPERM(:,:)
*********************************************************************
...
30    FORMAT(' zone t= "chamber",i=',I3,' , j=',I3,' , f= point') 
     ...
     1            TEMWRITE,XC(1:M+1,1),YC(1,1:N+1),M+1,N+1) 
      ...
      SUBROUTINE ADDMATERIAL
      IMPLICIT NONE
      INTEGER(KIND=4)::I,J
      ALLOCATE(FAR(M+1,N+1),FAI(M+1,N+1),CURR(M+1,N+1))
      ALLOCATE(ELECOND(M+1,N+1),DIELECOEF(M+1,N+1),MAGPERM(M+1,N+1))
      FAR=0.0;FAI=0.0;CURR=0.0
      OMIGA=1.0E+05
      ELECOND=5.0;DIELECOEF=1.0;MAGPERM=1.26E-06
*>CURRENT DENSITY
      DO J=1,N+1
      DO I=1,M+1
      IF(XC(I,J).GT.0.48.AND.XC(I,J).LT.0.52.AND.
     1   YC(I,J).GT.0.48.AND.YC(I,J).LT.052)THEN
      CURR(I,J)=1.2E+05
      ENDIF
      ENDDO
      ENDDO
...
*>PROJSOLA CONTROL PARAMETERS****************************************
      ...
      CALL BOUNDARYV
      CALL BOUNDARYP
*>SPLINE INTERPLATION,TEM(1:M+1,1:N+1)
      CALL READIN
      RETURN
      END SUBROUTINE INITIAL

      SUBROUTINE BOUNDARYA
      IMPLICIT NONE
      RETURN
      END SUBROUTINE BOUNDARYA

      SUBROUTINE SOLVEMAGVECPOTENTIAL
      IMPLICIT NONE
      INTEGER(KIND=4)::I,J
      ...
      REAL(KIND=8)::W(5*M*N)
      REAL(KIND=8)::DVAR(M+1,N+1),VAR1,VAR2,IRES=1.0E-6
      ISCONS=.FALSE.
      DO WHILE(.NOT.ISCONS)
      MBDCND=2;NBDCND=3
      BDA=0.0;BDB=0.0;BDC=0.0;BDD=0.0
      ELMBDA=-1.0
      F=0.0
      DO J=2,N
      DO I=2,M
      F(I,J)=-1.0*MAGPERM(I,J)*(CURR(I,J)+
     1                          DIELECOEF(I,J)*OMIGA**2*FAR(I,J)+
     1                          OMIGA*ELECOND(I,J)*FAI(I,J))
      ENDDO
      ENDDO
      CALL HWSCYL(A,B,M,MBDCND,BDA,BDB,
     1            C,D,N,NBDCND,BDC,BDD,
     1            ELMBDA,F,IDIMF,PERTRB,IERROR,W) 
      WRITE(*,'(A30,I4)')'SOLVE FAR ERROR=',IERROR
      DVAR=F-FAR;CALL MAXINMAT(DVAR,1,M+1,1,N+1,VAR1)
      FAR=F
      F=0
      DO J=2,N
      DO I=2,M
      F(I,J)=-1.0*MAGPERM(I,J)*(0.0+DIELECOEF(I,J)*OMIGA**2*FAI(I,J)-
     1                          OMIGA*ELECOND(I,J)*FAR(I,J))
      ENDDO
      ENDDO
      CALL HWSCYL(A,B,M,MBDCND,BDA,BDB,
     1            C,D,N,NBDCND,BDC,BDD,
     1            ELMBDA,F,IDIMF,PERTRB,IERROR,W) 
      WRITE(*,'(A30,I4)')'SOLVE FAI ERROR=',IERROR
      DVAR=F-FAI;CALL MAXINMAT(DVAR,1,M+1,1,N+1,VAR2)
      FAI=F
      WRITE(*,90)'FAR VAR1=',VAR1,,'FAI VAR2=',VAR2,'ISCONS=',ISCONS
      IF(MAX(ABS(VAR1),ABS(VAR2)).GT.IRES)THEN
      ISCONS=.FALSE.
      ELSE
      ISCONS=.TRUE.
      ENDIF
      ENDDO
90    FORMAT(1X,A10,ES15.5,A10,ES15.5,A10,L4)
      RETURN
      END SUBROUTINE SOLVEMAGVECPOTENTIAL
